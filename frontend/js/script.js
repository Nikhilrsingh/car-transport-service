...
function updateStepUI(stepNumber) {
  ...
  // OPTIMIZED: Direct targeted selector instead of traversing all DOM elements
  // This improves performance significantly, especially on complex pages
  const progressElement = document.getElementById("progressPercentage");
  if (progressElement) {
    progressElement.textContent = `${percent}% Complete`;
  } else {
    // Fallback: If ID not found, use more specific selector
    const progressText = document.querySelector(".progress-text, .step-progress-text");
    if (progressText) {
